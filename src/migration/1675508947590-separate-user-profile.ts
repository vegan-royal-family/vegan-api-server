...
export class separateUserProfile1675508947590 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        await queryRunner.query(`CREATE TABLE \`profile\` (\`id\` int NOT NULL AUTO_INCREMENT, \`userId\` int NOT NULL, \`nickname\` varchar(20) NOT NULL, \`gender\` enum ('MALE', 'FEMALE', 'UNKNOWN') NOT NULL, \`birth\` varchar(8) NOT NULL, \`veganFor\` json NOT NULL, \`veganLevel\` enum ('completely', 'preferably', 'interest') NOT NULL, \`veganTypeId\` int NOT NULL, \`profileFileId\` bigint NULL, \`createdAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`user\` ADD \`gender\` enum ('MALE', 'FEMALE', 'UNKNOWN') NULL`);
        await queryRunner.query(`ALTER TABLE \`user\` ADD \`veganLevel\` enum ('completely', 'preferably', 'interest') NULL`);
